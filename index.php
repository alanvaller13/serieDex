...
<header class="site-header">
    ...
    <div id="contagem-series" class="contagem-series"><?= count(array_slice($series, 0, $seriesPorPagina)) ?> de <?= $totalSeries ?> séries</div>

<div class="series-grid" id="series-container">
    <?php foreach (array_slice($series, 0, $seriesPorPagina) as $serie): ?>
...
<div class="loader" id="loader">
    ...
    <p>Carregando mais séries...</p>
</div>

<div class="no-more" id="no-more">
    <p>Você chegou ao fim da lista de séries.</p>
</div>

<script>
let currentPage = 1;
const seriesPerPage = <?= $seriesPorPagina ?>;
const totalSeries = <?= $totalSeries ?>;
let isLoading = false;
let allSeriesLoaded = false;

// Função para carregar mais séries
function loadMoreSeries() {
    if (isLoading || allSeriesLoaded) return;
    ...
    if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight - 500) {
        ...
        currentPage++;
        const nextPageUrl = `load_series.php?page=${currentPage}`;
        
        fetch(nextPageUrl)
            .then(response => response.text())
            .then(data => {
                if (data.trim() === '') {
                    allSeriesLoaded = true;
                ...
                const visibleSeries = document.querySelectorAll('.serie-card').length;
                document.getElementById('contagem-series').textContent = 
                    `Mostrando ${Math.min(visibleSeries, totalSeries)} de ${totalSeries} séries`;
                
                // Verifica se todas as séries foram carregadas
                if (visibleSeries >= totalSeries) {
                    allSeriesLoaded = true;
                    noMore.style.display = 'block';
                }
            })
...
window.addEventListener('scroll', loadMoreSeries);

// Carrega mais séries quando a página é carregada (caso a página inicial não preencha a tela)
...
function filtrarSeries() {
    ...
    document.getElementById('contagem-series').textContent = 
        `Mostrando ${visibleCount} de ${totalSeries} séries`;
}
