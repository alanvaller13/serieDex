...
.favorite-icon {
    position: absolute;
    top: 75px; /* 16px (top do streaming) + 48px (altura do streaming) + 6px (espaçamento) */
    left: 16px; /* Mesmo left do streaming-icon para alinhamento horizontal */
    width: 48px; /* Mesma largura para consistência */
    height: 48px;
    color: #003d9e;
    font-size: 2rem;
    background-color: rgba(255, 255, 255, 0.7); /* Fundo igual ao do streaming-icon */
    border-radius: 50%;
    display: flex;
    align-items: center;
    justify-content: center;
    z-index: 3;
    transition: all 0.3s ease; /* Adiciona transição suave como no streaming-icon */
}

/* Efeito de hover para combinar com o streaming-icon */
.serie-card:hover .favorite-icon {
    background: rgba(196, 185, 185, 0.9);
    transform: scale(1.1);
}

/* Ajuste opcional para o ícone interno (estrela) */
.favorite-icon i {
